...
function mapAssessmentRow(row) {
  ...
  return {
    ...
    questionsPresented: normalizeQuestions(row.questions_presented || row.questionsPresented || []),
    answers: row.answers || {},
    scores: row.scores || null,
    report: row.report || null,
    feedback: row.feedback || null,
    pageSize: Number(row.page_size || row.pageSize) || 3,
    totalQuestions: Number(row.total_questions || row.totalQuestions) || 10,
    pageCount: Number(row.page_count || row.pageCount) || Math.ceil((row.questions_presented || row.questionsPresented || []).length / (Number(row.page_size || row.pageSize) || 3)),
...
function toAssessmentPatchRow(patch) {
  ...
  if (patch.pageSize !== undefined) {
    row.page_size = Number(patch.pageSize) || 3;
  }
  if (patch.totalQuestions !== undefined) {
    row.total_questions = Number(patch.totalQuestions) || 10;
  }
  if (patch.pageCount !== undefined) {
    row.page_count = Number(patch.pageCount) || 4;
  }
