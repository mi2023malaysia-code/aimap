...
let STORE_PATH = path.join(__dirname, '..', 'data', 'assessments-runtime.json');
const TEMP_STORE_PATH = path.join(os.tmpdir(), 'assessments-runtime.json');
const STORE_OVERRIDE = String(process.env.ASSESSMENTS_RUNTIME_PATH || '').trim();
STORE_PATH = STORE_OVERRIDE || (process.env.VERCEL ? TEMP_STORE_PATH : STORE_PATH);
let useMemoryStore = false;
let memoryState = createInitialLocalState();
...
function switchToTempStore() {
  if (STORE_PATH === TEMP_STORE_PATH) {
    return false;
  }
...
function ensureLocalStore() {
  if (useMemoryStore) {
    memoryState = normalizeLocalState(memoryState);
    return;
  }
  ...
  if (!fs.existsSync(dir)) {
    try {
      ...
      if (isReadOnlyFilesystemError(error) && switchToTempStore()) {
...
function saveLocalState(state) {
  ensureLocalStore();
  if (useMemoryStore) {
    memoryState = normalizeLocalState(state);
    return;
  }
