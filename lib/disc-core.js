...
function normalizeQuestionEntry(question, fallbackOrder) {
  if (!question || typeof question !== 'object') {
    return null;
  }

  const id = String(
    question.id || question.question_code || question.questionId || question.code || ''
  ).trim();
  const type = String(question.type || question.question_type || '').trim().toUpperCase();
  const text = String(question.text || question.question_text || question.prompt || '').trim();

  if (!id || !type || !text) {
    return null;
  }

  const orderValue = Number(question.order !== undefined ? question.order : fallbackOrder);

  return {
    id,
    type,
    text,
    order: Number.isFinite(orderValue) && orderValue > 0 ? orderValue : fallbackOrder,
  };
}

function selectQuestions(questionPool = questionSeed) {
  const normalizedPool = normalizeQuestions(questionPool);
  const quotas = {
    D: 2,
    I: 2,
    S: 2,
  ...
  typeOrder.forEach((type) => {
    const pool = normalizedPool.filter((question) => question.type === type);
    ...
    .map((question, index) => {
      if (typeof question === 'string') {
        const base = questionBank.find((item) => item.id === question)
          || questionSeed.find((item) => item.id === question);
        if (!base) {
          return null;
        }
        return {
      ...
      return normalizeQuestionEntry(question, index + 1);
    })
    .filter(Boolean);
}
